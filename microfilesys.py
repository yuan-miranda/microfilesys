...
class Commands: # COMBINE ALL THE COMMANDS? 9/23
    ...
    def write(user_input):
        ...
        if length(user_input, '==', 4) and flag in ["-we", "--write-end"] and is_line(user_input[2]): # flag line content
            Write.write_end(flag, line = user_input[2], content = user_input[3]) # I JUST NEED TO PASS THE CONTENT ON WITH FUNCTION

        elif length(user_input, '==', 5) and flag in ["-ws", "--write-specific"] and is_line(user_input[2]): # flag line column content
    ...
    def modify(user_input):
        ...
        if length(user_input, '==', 5) and flag in ["-ms", "--modify-specific"] and is_line(user_input[2]): # flag line column content
            Modify.modify_specific(flag, line = user_input[2], column = user_input[3], content = user_input[4])

        elif length(user_input, '==', 4) and flag in ["-ma", "--modify-all"] and is_line(user_input[2]): # flag line content
            Modify.modify_all(flag, line = user_input[2], content = user_input[3])
                
        elif length(user_input, '==', 5) and flag in ["-mas", "--modify-all-specific"] and is_line(user_input[2]): # flag line content replace
    ...
    def remove(user_input):
        ...
        if length(user_input, '==', 4) and flag in ["-rmsub", "--remove-substring"] and is_line(user_input[2]): # flag line content
            Remove.remove_substring(flag, line = user_input[2], content = user_input[3])

        elif length(user_input, '==', 4) and flag in ["-rms", "--remove-specific"] and is_line(user_input[2]): # flag line column
            Remove.remove_specific(flag, line = user_input[2], column = user_input[3])

        elif length(user_input, '==', 4) and flag in ["-rmas", "--remove-all-specific"] and is_line(user_input[2]): # flag line content
    ...
    def clear(user_input):
        ...
        if length(user_input, '==', 3) and flag in ["-cl", "--clear-line"] and is_line(user_input[2]): # flag line
            Clear.clear_line(flag, line = user_input[2])

        elif length(user_input, '==', 2) and flag in ["-ca", "--clear-all"]: # flag
    ...
    def read(user_input):
        ...
        if length(user_input, '==', 3) and flag in ["-rl", "--read-line"] and is_line(user_input[2]): # flag line
            Read.read_line(line = int(user_input[2]))

        elif length(user_input, '==', 2) and flag in ["-ra", "--read-all"]: # flag
            Read.read_all()
    ...
    def close(user_input): # most cursed command structure ever
        ...
        if length(user_input, '==', 2) and flag in ["-c", "--close"]: # flag
            return Close.close()
...
def length(input, op, length):
    return (op == '==' and len(input) == length) or (op == '!=' and len(input) != length) or (op == '>' and len(input) > length) or (op == '<' and len(input) < length) or (op == '>=' and len(input) >= length) or (op == '<=' and len(input) <= length) # Java be like
...
def file_editor(filepath):
    ...
    while running:
        try:
            ...
            f_user_input = make_list(user_input)
        ...
        elif length(f_user_input, '==', 1) and cmd in file_editor_cmd_handler:
        ...
        # throw an error if user type a non-command input
            ...
            print(f"'{' '.join(f_user_input)}' is not a valid file editor command")
        ...
        # call the appopriate command when it pass all the conditions above
            ...
            file_editor_cmd_handler[cmd](f_user_input)
...
def file_manager():
    ...
    while running:
        ...
        # prompt to use the command manual if input is valid command but
        # only the command is being called
        elif length(f_user_input, '==', 1) and cmd in file_manager_cmd_handler:
            print(Errors.error_messages["invalid command"][cmd])
        
        # throw an error if user type a non-command input
        elif not cmd in file_manager_cmd_handler:
            print(f"'{' '.join(f_user_input)}' is not a valid file manager command")
        
        # call the appopriate command when it pass all the conditions above
        else:
            file_manager_cmd_handler[cmd](f_user_input)
